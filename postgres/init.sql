...
IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'webmaster') THEN
    CREATE USER webmaster WITH ENCRYPTED PASSWORD 'bmrbnmr' CREATEDB;
END IF;
...
SELECT 'CREATE DATABASE internal OWNER webmaster' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'internal') \gexec
...
CREATE TABLE IF NOT EXISTS session (
    token               UUID PRIMARY KEY DEFAULT uuidv7(),  -- token used by user and web frontend, uuidv7() func requires PosgreSQL 18+.
    token_admin         UUID DEFAULT gen_random_uuid(),     -- token used by administrators to respond to inquiries on 'Communicationi' page
    token_expiry        TIMESTAMP NOT NULL,                 -- time of expiry of the session

    gdpr_consent        BOOLEAN NOT NULL DEFAULT FALSE,     -- whether user approved GDPR terms
    gdpr_consent_time   TIMESTAMP,                          -- timestamp when user approve GPDR terms
    ...
    approved            BOOLEAN DEFAULT FALSE,  -- timestamp when user acknowledges all warnings, allowing user to download the conversion results.
    transfered          BOOLEAN DEFAULT FALSE,  -- timestamp when data exchange among production servers completes.
    downloaded          BOOLEAN DEFAULT FALSE   -- timestamp when user downloads the conversion results.
);
...
CREATE TYPE wf_task_code as ENUM ('issue_conversion', 'convert_model', 'convert_nmr_data', 'session_exchange', 'session_lock', 'session_cleanup', 'notification', 'communication');
...
SELECT 'CREATE DATABASE prefect OWNER webmaster' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'prefect') \gexec
